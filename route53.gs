...
var exports = (function(){
  ...
  function menuSyncHandler(e) {
    console.log({message:'synchandler', initialData:e});
    ...
    app.btns=[{btn:'btn_continue', visible:false }];
    return app;
  }
  ...
  function menuSubmitHandler(e) {
    ...
    app.btns=[{btn:'btn_continue', visible:false },
              {btn:'btn_checkstatus', visible:results.length > 0 }];
    return app;
  }
  ...
  function menuCreateZone() {
    if(menuConfigOk()) {
      var btns = [
        ...
        { label:'Check Status', func:'menuCheckHandler',  visible:false }];
      var fields = [{ label:'Domain name:', name: 'domain' }];
      requestOpen('Create new zone', msgCreate1, '', btns, fields);
    }
  }
  ...
  function menuCreateHandler(e) {
    ...
    try {
      ...
      app.btns=[{btn:'btn_create', visible:false },
                {btn:'btn_checkstatus', visible:true }];
      return app;
    } catch(e) {
  ...
  function menuDeleteZone() {
    if(menuConfigOk()) {
      ...
      var btns = [
        ...
        { label:'Check Status', func:'menuCheckHandler',  visible:false }];
      if(!zone.isDeletable()) {
        Browser.msgBox(msgDelete2);
        return null;
      } else {
        requestOpen('Delete this zone', msgDelete1, Utilities.jsonStringify(zone.toJson()), btns);
      }
    }
  }
  ...
  function menuDeleteHandler(e) {
    ...
    try {
      ...
      app.btns=[{btn:'btn_delete', visible:false },
                {btn:'btn_checkstatus', visible:true }];
      return app;
    } catch(e) {
      return requestUpdate('Failed to create the hosted zone.', ''+e, null);
  ...
  function requestOpen(title, msg, params, btns, fields) {
    var t = HtmlService.createTemplateFromFile('htmlRequestOpen');
    btns.push({ label: 'Close', func:'closeModal', visible:true });
    t.myBtns=btns;
    t.myMsg=msg;
    t.myParams=params;
    if (!fields) { fields=[] };
    ...
    SpreadsheetApp.getUi()
      .showModalDialog(t.evaluate(),title)
...
//  }
  ...
  var Config = {
    ...
    secret: PropertiesService.getDocumentProperties().getProperty('secret'),
    save: function() {
      for(var i in this) {
        if(typeof this[i] != 'function') {
          PropertiesService.getDocumentProperties().setProperty(i, this[i]);
        }
      }
    },
    isOk: function() {
  ...
  function configOpen() {
    var t = HtmlService.createTemplateFromFile('htmlConfigOpen');
    SpreadsheetApp.getUi()
      .showModalDialog(t.evaluate(),'Route53 Settings')
  }

  function configHandler(parameter) {
    if(parameter.key && parameter.secret) {
      Config.key    = parameter.key;
      Config.secret = parameter.secret;
      Config.save();
      Logger.log('Config saved!');
    } else {
      Logger.log('configOk called with missing params');
    }
  }
  ...
  function infoOpen(title, text) {
    var t = HtmlService.createTemplateFromFile('htmlInfoOpen');
    t.myText = text;
    SpreadsheetApp.getUi()
      .showModalDialog(t.evaluate(),title)
  }
...
function configHandler()          { return exports.configHandler.apply(this, arguments); }
